...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    class AutoDriver {
      mouse: MouseClass; manager: WebGLManager;
    ...
    class ShaderPass {
      ...
      constructor(props: Record<string, unknown>) {
        this.props = props || {};
        this.uniforms = (this.props.material as Record<string, Uniforms> | undefined)?.uniforms;
      }
      init() {
    ...
    class Advection extends ShaderPass {
      ...
      constructor(simProps: Record<string, unknown>) {
        ...
        this.uniforms = (this.props.material as Record<string, Uniforms>).uniforms;
        this.init();
      }
      init() {
        super.init();
        ...
        this.line = new THREE.LineSegments(boundaryG, new THREE.RawShaderMaterial({ vertexShader: line_vert, fragmentShader: advection_frag, uniforms: this.uniforms! }));
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(props: { mouse_force?: number; cursor_size?: number; cellScale?: THREE.Vector2 }) {
        ...
        const cs = props.cellScale || new THREE.Vector2(1, 1);
        const cur = props.cursor_size || 0;
        const uniforms = (this.mouse.material as THREE.RawShaderMaterial).uniforms;
        uniforms.force.value.set(forceX, forceY);
        uniforms.center.value.set(
          Math.min(Math.max(Mouse.coords.x, -1 + cur * cs.x + cs.x * 2), 1 - cur * cs.x - cs.x * 2),
          Math.min(Math.max(Mouse.coords.y, -1 + cur * cs.y + cs.y * 2), 1 - cur * cs.y - cs.y * 2),
        );
        uniforms.scale.value.set(cur, cur);
        super.update();
      }
    }
    ...
    class Simulation {
      ...
      update() {
        ...
        if (this.options.isViscous) vel = this.viscous.update({ viscous: this.options.viscous, iterations: this.options.iterations_viscous, dt: this.options.dt }) ?? vel;
